<script lang="ts">
	...
	onMount(() => {
		...
		// Reveal animation observer - consistent with other sections
	...
	// Funkcja do ikon - POPRAWIONE IKONY
	function getIcon(iconType: string) {
		const icons: Record<string, string> = {
			"audit": `<svg class="h-10 w-10 text-brand-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
				<path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
			</svg>`,
			"implementation": `<svg class="h-10 w-10 text-brand-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
				<path stroke-linecap="round" stroke-linejoin="round" d="M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 004.486-6.336l-3.276 3.277a3.004 3.004 0 01-2.25-2.25l3.276-3.276a4.5 4.5 0 00-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437l1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008z" />
			</svg>`,
			"support": `<svg class="h-10 w-10 text-brand-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
		...
		<swiper-slide>
			<div class="timeline-card card group flex flex-col p-8 bg-white border border-slate-200/80 rounded-3xl shadow-sm h-full transition-all duration-500 hover:border-brand-blue/50 hover:shadow-xl hover:shadow-brand-blue/10">
				<div class="flex items-center justify-between mb-4">
					<!-- Enhanced icon container - IDENTICAL TO PROBLEMS/SERVICES -->
				...
				<div class="flex flex-col flex-grow">
					...
					<p class="text-text-muted mb-4 leading-relaxed group-hover:text-text-primary/90 transition-colors duration-300">
						{step.description}
					</p>
					...
					<ul class="text-text-muted space-y-2 flex-grow">
						{#each step.details as detail}
							<li class="flex items-start detail-item group-hover:text-text-primary/90 transition-colors duration-300">
								<svg class="w-5 h-5 mr-2 text-brand-blue flex-shrink-0 transition-transform duration-300 group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<style>
	/* === IDENTICAL STYLING TO PROBLEMS/SERVICES SECTIONS === */
	
	/* 1. REVEAL ANIMATIONS - SAME AS OTHER SECTIONS */
	...
	:global(.timeline-card.revealed) {
		opacity: 1;
		transform: translateY(0) scale(1);
	}

	/* 2. ENHANCED CARD INTERACTIONS - IDENTICAL TO OTHER SECTIONS */
	:global(.timeline-card) {
		cursor: default;
		position: relative;
		min-height: 420px;
	...
	/* 3. ICON GLOW EFFECT - SAME AS PROBLEMS/SERVICES */
	.icon-glow {
	...
	/* 4. ENHANCED HOVER DEPTH - SAME AS OTHER SECTIONS */
