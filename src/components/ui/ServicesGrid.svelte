<script lang="ts">
    ...
    // USUNIĘTO onMount - logika animacji nie jest już potrzebna w tym komponencie
    ...
    // CAŁY BLOK onMount ZOSTAŁ USUNIĘTY
</script>
...
<style>
    /* === USUNIĘTO SEKCJĘ Z ANIMACJĄ WEJŚCIA === */
    /* Usunięto reguły dla :global(.service-card) i :global(.service-card.revealed) */

    /* Pozostałe style interakcji - bez zmian */
    .service-card {
        cursor: pointer;
        position: relative;
    }
    ...
    .service-card:hover {
        /* Poprawka: usunięto scale(1), aby uniknąć konfliktu z animacją wejścia */
